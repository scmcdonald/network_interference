...
final_summ <- final_df %>%
  ...
  relocate( `dis1_1e-04`, .before = dis1_0.001)


best <- apply(X = final_summ[, c("dis1_1e-04",  "dis1_0.001", "dis1_0.1","dis1_1")], MARGIN = 1, FUN =function(x){ which(x == min(x))}) %>%
  lapply(., names)

final_summ$`min_1e-04` <- sapply(best, function(x)is.element("dis1_1e-04", x), simplify = T) %>% as.numeric
...
final_summ %>%
  group_by(min_1, min_0.1, min_0.001, `min_1e-04`, p_num) %>%
  count()
```
