...
Note: we removed $\rho_1 = $ standard normal and normal(0.1, 1) because they contain zero and were causing errors.
...
ridge <- function(output, lambda){
  ...
  dis <- disagreements(output$comparison_matrix, g_theta(A, theta = 0.1), output$N)
  ...
  diagnostics <- rbind(N= output$N, periods = output$periods_type, rankY = output$rankY, 
                       ...
                       error_message = error_message, seed = output$seed, lambda = lambda, 
                       disagreements = dis)
...
for(i in 1:nrow(tests)){
  for(j in 1:10){  
    ...
    ls_out <- ridge(output, lambda = tests[i, "lambda"])
...
error_rate_ridge <- final_seed_test %>%
  ...
  mutate(ave_dis = as.character(mean(as.numeric(disagreements)))) %>%
  group_by(N, periods, s_alpha, s_epsilon, s_beta, p_num, rho_1, rho_2, y_initial, error, lambda, ave_dis) %>%
  count(name = "error_count") %>%
  ungroup() %>%
  filter(error == "no error") %>%
  mutate(error_rate  = (10 - error_count)/10) %>%
  select(!error &  !error_count) %>%
...
N = 20
output <- dgp(N= N, 
              periods_type = "squared", 
              s_alpha = 0.2, s_beta = 0, s_epsilon = 0.3, 
              rho_1 = "unif(.9, 1)", rho_2 = "zero", 
              y_initial = "standard normal", seed = 1, 
              p_num = "low")

out <- ridge(output = output, lambda = 0.000001)
out$diagnostics
```
