...
compare_10$prediction_date <- as.Date(compare_10$week_end) + 7


text_color <- compare_10 %>%
...
compare_10 %>%
  ...
  scale_color_manual(name = "RMSE", values = c(cb_colors[1:4], "black"), 
                     labels = c("Basic", "HLAGC", "HALGELEM", "HLAGOO", "Week Means")) +
  scale_x_reverse(labels = as.character(format(compare_10$prediction_date, format = "%b %d")),                     
                  breaks = as.numeric(compare_10$prediction_date)) + 
  scale_y_continuous(limits = c(0,5))+ 
  theme_minimal() +
  coord_flip()  +
  labs(x = "Date (2021)", 
